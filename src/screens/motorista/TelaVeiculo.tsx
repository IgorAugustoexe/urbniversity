...
export default function TelaVeiculo() {
  ...
  const image =
    (store.user.type == 'driver' && store.user.user.photo) ||
  ...
  const ImagemVeiculo = () => (
    <Image style={styles.imgVeiculo} source={{uri: `${image}`}} />
  );
      ...
      <View style={styles.cabecalho}>
        ...
        {loaderReq ? (
          <ActivityIndicator
            size={'large'}
            color={cores.branco}
            style={{paddingVertical: config.windowWidth / 5}}
          />
        ) : (
          <Fragment>
    ...
    <SafeAreaView style={estilos.containerPrincipal}>
      ...
      {solicitar && (
        <TouchableOpacity
          ...
          disabled={loaderBtn}
          style={styles.btnRodape}>
          <BtnBlue text={'SOLICITAR MOTORISTA'} loader={loaderBtn} />
        </TouchableOpacity>
      )}
    </SafeAreaView>
  );
