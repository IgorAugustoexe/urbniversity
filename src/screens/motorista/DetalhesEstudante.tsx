...
import { faGraduationCap, faLocationDot, faPhone, faSchoolFlag, faTreeCity, faXmark } from '@fortawesome/free-solid-svg-icons'
...
export default function DetalhesEstudante() {
    ...
    const removeEstudante = async () => {
        await mediador('Deseja mesmo remover este estudante?', removeFromRoute, route.params.id)
    }
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            <View style={styles.container}>
                <TouchableOpacity style={{ alignSelf: 'flex-end' }} onPress={() => navigation.goBack()}>
                    <FontAwesomeIcon icon={faXmark} size={config.windowWidth / 12} color={cores.branco} />
                </TouchableOpacity>
                <View style={{ alignItems: 'center' }}>
                    <Image
                        style={styles.imgEstudante}
                        source={route.params.imagem ? { uri: route.params.imagem } : avatarPadrao}
                    />
                </View>
                <View style={{ borderBottomWidth: 0.5, borderColor: cores.branco, paddingTop: 20, marginHorizontal: config.windowWidth / 4 }} />
                <View style={styles.containerInfoEstudante}>
                    <View style={styles.divisorIconeTxt}>
                        <FontAwesomeIcon icon={faTreeCity} size={config.windowWidth / 16} color={cores.branco} />
                        <Text style={styles.txtInfo}>{route.params.cidade}</Text>
                    </View>
                    <View style={styles.divisorIconeTxt}>
                        <FontAwesomeIcon icon={faLocationDot} size={config.windowWidth / 16} color={cores.branco} />
                        <Text style={styles.txtInfo}>{route.params.endereco}</Text>
                    </View>
                    <View style={styles.divisorIconeTxt}>
                        <FontAwesomeIcon icon={faSchoolFlag} size={config.windowWidth / 16} color={cores.branco} />
                        <Text style={styles.txtInfo}>{route.params.faculdade}</Text>
                    </View>
                    <View style={styles.divisorIconeTxt}>
                        <FontAwesomeIcon icon={faGraduationCap} size={config.windowWidth / 16} color={cores.branco} />
                        <Text style={styles.txtInfo}>{route.params.nome}</Text>
                    </View>
                    <View style={[styles.divisorIconeTxt, { paddingBottom: 0 }]}>
                        <FontAwesomeIcon icon={faPhone} size={config.windowWidth / 18} color={cores.branco} />
                        <Text style={styles.txtInfo}>{route.params.celular}</Text>
                    </View>
                </View>
                <TouchableOpacity style={styles.btnRemover} onPress={() => removeEstudante()}>
...
const styles = StyleSheet.create({
    container: {
        ...
        height: config.windowWidth / 0.78,
        borderRadius: 7,
